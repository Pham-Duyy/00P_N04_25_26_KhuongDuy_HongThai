...
class GroupAdminControllerTest {
    ...
    // ===================== TẠO NHÓM =====================
    @Test
    void testCreateGroup_WhenAdminLoggedIn_ShouldCreateGroupAndSendInvites() throws Exception {
        List<User> allUsers = Arrays.asList(adminUser, createUser(2L, "user1", User.Role.USER));
        when(userService.findAllUsers()).thenReturn(allUsers);
        doNothing().when(groupService).addGroup(any(Group.class));
        doNothing().when(groupInviteService).sendInvite(anyLong(), anyLong());
        ...
        verify(groupInviteService, atLeastOnce()).sendInvite(anyLong(), anyLong());
    }
